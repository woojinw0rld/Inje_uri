...
import { chat_room_source_type, Prisma } from "@/generated/prisma/client";
...
export async function createChatRoom(input: CreateChatRoomParams,
  ...
  const existing = await
    chatRoomRepo.findActiveRoomBetweenUsers(requestUserId, targetUserId, matchTransaction);
  if (existing) {
    return { error: ERROR.DUPLICATE_ACTIVE_ROOM } as const;
  }

  // 2. 재매칭 7일 정책
  const lastLeft = await chatRoomRepo.findLastLeftRoomBetweenUsers(requestUserId, targetUserId, matchTransaction);
  if (lastLeft) {
