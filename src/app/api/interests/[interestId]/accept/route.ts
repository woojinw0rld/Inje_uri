...
import { acceptInterest } from "@/server/services/matching/interest.service";
...
export async function POST(
  ...
  try {
    ...
    const data = await acceptInterest(userId, parsedInterestId);
    return ok(data);
  } catch (e) {
    if (e instanceof ApiError) {
      return fail(e.code, e.message);
    }
    console.error("[POST /api/interests/[interestId]/accept]", e);
