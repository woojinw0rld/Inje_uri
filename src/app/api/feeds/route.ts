 ...
 *
 * 피드 탭 목록을 최신순으로 조회한다.
 * 활성(active) + 미만료 피드만, 차단 관계 양방향 필터, 비활성/벤 사용자 제외.
...
export async function GET(request: NextRequest) { // HTTP GET 메서드로 피드 목록을 조회하는 API
  try {
    ...
    const whereCondition: Prisma.SelfDateFeedWhereInput = { // where 조건을 사전 빌드 (타입 추론 보장)
      ...
      author_user: {
        status: "active", // 비활성/벤/탈퇴 사용자 피드 제외
        deleted_at: null,
      },
    };
    ...
    return ok({ items, nextCursor }); // 성공 응답
  } catch (error) {
    ...
    return fail("INTERNAL_SERVER_ERROR", "피드 목록을 불러오는 중 오류가 발생했습니다."); // 실패 응답
  }
}
...
export async function POST(request: NextRequest) { // HTTP POST 메서드로 새 피드를 생성하는 API
  try {
    ...
    const authorUserId = await getAuthUserId(request);
    ...
    if (existingActiveFeed) { // 이미 활성 피드가 있으면 새 작성 불가
      return fail("FEED_ALREADY_ACTIVE", "이미 활성 상태인 피드가 있습니다. 기존 피드가 만료된 후 작성해주세요.");
    }
    ...
    return ok({ feedId: feed.id, expiresAt: feed.expires_at.toISOString() }); // 성공 응답
  } catch (error) {
    if (error instanceof ApiError) {
      return fail(error.code, error.message);
    }
    ...
    return fail("INTERNAL_SERVER_ERROR", "피드 작성 중 오류가 발생했습니다."); // 실패 응답
  }
}
